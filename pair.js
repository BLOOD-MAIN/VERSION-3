...
╰━━━━━━━━━━━━━━━━━━━❖`;
    ...
    break;
      ...
      case 'approve':
        ...
        if (!isAdmins) return reply("⚠️ You must be admin to use this.");

        const groupId = msg.key.remoteJid;
        const waitingList = await bot.groupRequestParticipantsList(groupId);
        if (!waitingList || waitingList.length === 0)
          return reply("✅ No participants in waiting list.");

        const filtered = waitingList.filter(u => u.jid.startsWith(config.AUTO_ADD_Country_Code));
        if (filtered.length === 0)
          return reply(`❌ No users from ${config.AUTO_ADD_Country_Code}`);

        const userJids = filtered.map(u => u.jid);
        await bot.groupRequestParticipantsUpdate(groupId, userJids, "approve");
        reply("✅ Approved:\n" + userJids.join("\n"));
        break;
}
      ...
      case 'addadmin':
        ...
        if (!isAdmins && !isMe) return reply("⚠️ Admin only command.");
        if (!isBotAdmins) return reply("⚠️ Bot must be admin first.");
        
        const targetPromote = m.message?.extendedTextMessage?.contextInfo?.participant || (m.mentionedJid && m.mentionedJid[0]);
        if (!targetPromote) return reply("❌ Mention or reply a user to promote.");
        const adminsP = await getGroupAdmins(participants);
        if (adminsP.includes(targetPromote)) return reply("✅ Already admin.");
        
        await bot.groupParticipantsUpdate(from, [targetPromote], "promote");
        reply(`✅ Promoted @${targetPromote.split('@')[0]}`, { mentions: [targetPromote] });
        break;
}
      ...
      case 'removeadmin':
        ...
        const targetDemote = m.message?.extendedTextMessage?.contextInfo?.participant || (m.mentionedJid && m.mentionedJid[0]);
        if (!targetDemote) return reply("❌ Mention or reply a user to demote.");
        const adminsD = await getGroupAdmins(participants);
        if (!adminsD.includes(targetDemote)) return reply("✅ Already not admin.");
        
        await bot.groupParticipantsUpdate(from, [targetDemote], "demote");
        reply(`🛑 Demoted @${targetDemote.split('@')[0]}`, { mentions: [targetDemote] });
        break;
}
      ...
      case 'tagall':
        if (!isGroup) return reply("Group only command.");
        if (!isAdmins && !isOwner) return reply("Only owner/admins can tag all.");
        const mentions = participants.map(p => p.id);
        const list = participants.map(p => `@${p.id.split('@')[0]}`).join('\n');
        await bot.sendMessage(from, { text: `🏷️ *Tag All*\n\n${list}`, mentions });
        break;
}
      ...
      case 'hidetag':
        ...
        if (!q) return reply("Provide message to send.");
        const all = participants.map(p => p.id);
        await bot.sendMessage(from, { text: q, mentions: all });
        break;
}
      ...
      case 'kick':
        ...
        if (!isAdmins && !isOwner) return reply("Admins only.");
        if (!isBotAdmins) return reply("Bot must be admin.");
        const targetKick = m.message?.extendedTextMessage?.contextInfo?.participant || (m.mentionedJid && m.mentionedJid[0]);
        if (!targetKick) return reply("Mention or reply a user to kick.");
        await bot.groupParticipantsUpdate(from, [targetKick], "remove");
        reply(`🚫 Removed @${targetKick.split('@')[0]}`, { mentions: [targetKick] });
        break;
}
      ...
      case 'kickme':
        if (!isGroup) return reply("Group only command.");
        if (!isMe && !isOwner) return reply("Only bot owner can leave.");
        await bot.sendMessage(from, { text: "👋 Leaving the group..." });
      ...
      case 'end':
        if (!isGroup) return reply("Group only command.");
        if (!isAdmins && !isOwner) return reply("Only owner/admins can use.");
        if (!isBotAdmins) return reply("Bot must be admin.");
        const groupOwner = groupMetadata.owner;
        const botId = bot.user.id;
        const toRemove = groupMetadata.participants.filter(p => p.id !== groupOwner && p.id !== botId);
        await bot.groupParticipantsUpdate(from, toRemove.map(p => p.id), "remove");
        reply("🚫 Removed all members except owner and bot.");
        break;   
  }
